...
import static veribaseplugin.util.ConfigUtil.*;

@Slf4j
@Singleton
@RequiredArgsConstructor(onConstructor_ = { @Inject })
public class SettingsManager {
    public static final String CONFIG_GROUP = "veribaseplugin";
    ...
    private final VeribasePlugin plugin;
    ...
    public void init() {
        ...
        webhookConfigKeys = ImmutableSet.<String>builder()
            .add("discordWebhook") // VeribasePluginConfig#primaryWebhook
            //.addAll(keysBySection.getOrDefault(VeribasePluginConfig.webhookSection.toLowerCase().replace(" ", ""), Collections.emptySet()))
    ...
    void onCommand(CommandExecuted event) {
        ...
        if ("VeribaseHash".equalsIgnoreCase(cmd)) {
            CompletableFuture.completedFuture(client.getAccountHash())
                ...
                .thenRun(() -> plugin.addChatSuccess("Copied your veribase hash to clipboard"))
                .exceptionally(t -> {
                    plugin.addChatWarning("Failed to copy your veribase hash to clipboard");
                    return null;
                });
        } else if ("VeribaseInit".equalsIgnoreCase(cmd)) {
        ...
        if (value != null && value.isEmpty() && ("embedFooterText".equals(key) || "embedFooterIcon".equals(key) || "deathIgnoredRegions".equals(key) || ChatNotifier.PATTERNS_CONFIG_KEY.equals(key))) {
            SwingUtilities.invokeLater(() -> {
                if (StringUtils.isEmpty(configManager.getConfiguration(CONFIG_GROUP, key))) {
                    // non-empty string so runelite doesn't overwrite the value on next start; see https://github.com/pajlads/VeribasePlugin/issues/453
    ...
    /**
     * Exports the full Veribase config to a JSON map, excluding empty lists,
    ...
    /**
     * Imports a Veribase JSON config export from the user's clipboard
     */
    @Synchronized
