...
import veribaseplugin.message.Embed;
import veribaseplugin.message.NotificationBody;
import veribaseplugin.message.NotificationType;
import veribaseplugin.message.templating.Evaluable;
import veribaseplugin.message.templating.Replacements;
import veribaseplugin.message.templating.Template;
import veribaseplugin.message.templating.impl.JoiningReplacement;
import veribaseplugin.notifiers.data.LootNotificationData;
import veribaseplugin.notifiers.data.SerializedItemStack;
import veribaseplugin.util.ConfigUtil;
import veribaseplugin.util.ItemUtils;
import veribaseplugin.util.KillCountService;
import veribaseplugin.util.MathUtils;
import veribaseplugin.util.RarityService;
...
public class LootNotifier extends BaseNotifier {
    ...
    public void onNpcLootReceived(NpcLootReceived event) {
        ...
        System.out.println("LOOT EVENT!!!: " + event);
    ...
    private void handleNotify(Collection<ItemStack> items, String dropper, LootRecordType type) {
        ...
        if (!sendMessage) {
            if (sufficientlyRare(rarest)) {
                ...
                // if no single item itself would exceed the min value config - github.com/pajlads/VeribasePlugin/issues/403
                sendMessage = true;
                lootMsg = Replacements.ofMultiple(" ",
                    Replacements.ofText("Various items including:"),
