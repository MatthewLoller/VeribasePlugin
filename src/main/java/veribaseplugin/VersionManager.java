...
public class VersionManager {
    ...
    @Inject
    private VeribasePlugin plugin;

    @Inject
    private VeribasePluginConfig config;
    ...
    void onStart() {
        if (config.pluginVersion().isEmpty()) {
            if (settingsManager.hasModifiedConfig()) {
                // Veribase was already installed/used & launched for the first time since VersionManager was created
                setStoredVersion("1.8.4");
            } else {
                // first time launching Veribase; no chat message necessary
