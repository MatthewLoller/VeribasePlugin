...
import veribaseplugin.MockedTestBase;
...
import veribaseplugin.util.BlockingClientThread;
import veribaseplugin.util.BlockingExecutor;
import veribaseplugin.util.TestImageUtil;
...
abstract class MockedNotifierTest extends MockedTestBase {
    ...
    @Bind
    protected VeribasePluginConfig config = Mockito.mock(VeribasePluginConfig.class);
    ...
    @Bind
    protected VeribasePlugin plugin = Mockito.spy(VeribasePlugin.class);

    @Bind
    protected ConfigManager configManager = Mockito.mock(ConfigManager.class);

    @Bind
    protected SettingsManager settingsManager = Mockito.spy(new SettingsManager(gson, client, clientThread, plugin, config, configManager, discordService));
    ...
    protected void setUp() {
        ...
        when(config.embedFooterText()).thenReturn("Powered by Veribase");
        when(config.embedFooterIcon()).thenReturn("https://github.com/pajlads/VeribasePlugin/raw/master/icon.png");
